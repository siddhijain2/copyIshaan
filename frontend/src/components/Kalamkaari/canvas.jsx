...
>>>>>>> 420d3d78480b746cfc288565b60fe270027284d6
  ...
  render() {
    ...
    const sizeList = canvasSizes.map((size) => {
      return (
        <option
          className="canvas-size-dropdown-options"
          value={size}
          key={size}
        >
          {size}
    ...
    const disablePredBtn =
      this.state.predictionProgress === "" && this.state.predBtnCountdown === 0
        ? false
        : true;

    const predCountdown =
      this.state.predBtnCountdown === 0
        ? ""
    ...
    return (
      <div className="canvas my-36">
        ...
        {this.state.predictionProgress ? (
          ...
          <h5 className="prediction-result">
