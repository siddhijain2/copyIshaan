...
function getTextReadedDiff(original, readed) {
  ...
  let lastPartMentioned = _.findLastIndex(
    diff,
    (part) => isAddedPart(part) || isUnchangedPart(part)
  );

  const isNextPartPunctuation =
    diff[lastPartMentioned + 1] &&
    diff[lastPartMentioned + 1].value.length <= 2;
  if (isNextPartPunctuation) {
    lastPartMentioned = lastPartMentioned + 1;
  }
  const lastPartMentionedIsBeforeLast =
    lastPartMentioned + 1 === diff.length - 1;
...
export function getScore(computedFeedback) {
  const similarityByWord = computedFeedback.reduce((sentences, sentence) => {
    if (sentence.value !== " ") {
      const wordsSimilarity = sentence.value
        .split(" ")
